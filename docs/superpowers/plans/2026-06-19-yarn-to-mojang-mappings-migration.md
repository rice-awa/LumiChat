...
| `net.minecraft.server.command.CommandOutput` | `net.minecraft.commands.CommandSource` | ⚠️ Task 0 复核 |
...
| `net.minecraft.command.permission.PermissionCheck` | `net.minecraft.commands.CommandSourceStack` 相关 ⚠️ Task 0 复核（1.21.11 Mojang 下权限 API 形态） | ⚠️ |
...
| `net.minecraft.world.GameRules` | `net.minecraft.world.level.GameRules` | ⚠️ Task 0 复核（1.21.11 Mojang 位置） |
| `net.minecraft.world.rule.GameRules`（1.21.11 Yarn） | `net.minecraft.world.level.GameRules` | ⚠️ Task 0 复核 |
...
| `net.minecraft.entity.mob.HostileEntity` | `net.minecraft.world.entity.monster.Monster` | ⚠️ Task 0 复核 |
| `net.minecraft.entity.passive.PassiveEntity` | `net.minecraft.world.entity.AgeableMob` | ⚠️ Task 0 复核 |
| `net.minecraft.entity.effect.StatusEffectInstance` | `net.minecraft.world.effect.MobEffectInstance` | ✅ |
| `net.minecraft.entity.SpawnReason` | `net.minecraft.world.entity.MobSpawnType` | ⚠️ Task 0 复核 |
...
| `Text.of(s)` | `Component.nullToEmpty(s)` | ⚠️ 复核 |
| `Formatting.X` | `ChatFormatting.X` | ✅ |
| `ServerCommandSource.sendFeedback(supplier, bool)` | `CommandSourceStack.sendSuccess(supplier, bool)` | ⚠️ 复核（Mojang 1.21.x 形态） |
| `Identifier.tryParse` | `ResourceLocation.tryParse` | ✅ |
| `Identifier.of(ns, path)` | `ResourceLocation.fromNamespaceAndPath(ns, path)`（1.21+）/ `new ResourceLocation(ns, path)`（旧） | ✅ |
| `Identifier.of(id)` | `ResourceLocation.parse(id)`（1.21+）/ `new ResourceLocation(id)`（旧） | ⚠️ 复核 |
| `new Vec3d(x,y,z)` | `new Vec3(x,y,z)` | ✅ |
| `Registries.BLOCK.get(id)` | `BuiltInRegistries.BLOCK.get(id)` | ✅ |
| `world.getEntitiesByClass(...)` | `level.getEntitiesOfClass(...)` | ✅ |
| `world.getTopY(...)` | `level.getHeight(...)` | ⚠️ 复核 |
| `world.getTimeOfDay()` | `level.getDayTime()` | ⚠️ 复核 |
| `world.setTimeOfDay(v)` | `level.setDayTime(v)` | ⚠️ 复核 |
| `entity.getEyePos()` | `entity.getEyePosition()` | ✅ |
| `server.getOverworld()` | `server.overworld()` | ⚠️ 复核 |
| `server.getCommandManager()` | `server.getCommands()` | ✅ |
| `effect.getEffectType()` | `effect.getEffect()` | ⚠️ 复核（1.20.5+ `.value()` 链） |
| `player.getStatHandler()` | `player.getStats()` | ⚠️ 复核 |
| `@Accessor("world")`（ServerPlayerInteractionManager 字段） | `@Accessor("level")` | ⚠️ Task 0 复核字段名 |
| `@Accessor("server")`（ServerPlayer 字段） | `@Accessor("server")` | ⚠️ Task 0 复核字段名 |
...
`import net.minecraft.util.Identifier;` → `import net.minecraft.resources.ResourceLocation;`。文件内所有 `Identifier` → `ResourceLocation`。方法体内按重命名表：
  - `Identifier.tryParse(id)` → `ResourceLocation.tryParse(id)`
  - `Identifier.of("minecraft", id)` → `ResourceLocation.fromNamespaceAndPath("minecraft", id)`（1.21+ 分支）/ `new ResourceLocation("minecraft", id)`（旧分支）
  - `Identifier.of(namespace, path)` → `ResourceLocation.fromNamespaceAndPath(namespace, path)` / `new ResourceLocation(namespace, path)`
  - `Identifier.of(id)` → `ResourceLocation.parse(id)`（1.21+）/ `new ResourceLocation(id)`（旧）—— **以 Task 0 复核结果为准**

**保留** `//? >=1.21 { ... //?} else { /*...*//*?}}` 结构不变，仅替换内部 `Identifier`/`ResourceLocation` 调用。

- [ ] **Step 2: 更新返回类型与 Javadoc 中的类名**

方法签名返回类型 `Identifier` → `ResourceLocation`；Javadoc 中"Identifier"措辞改为"ResourceLocation"。

- [ ] **Step 3: 提交**

```bash
git add src/main/java/com/riceawa/llm/compat/IdentifierCompat.java
git commit -m "refactor(mappings): IdentifierCompat 迁移至 ResourceLocation（Mojang）"
```
...
- [ ] **Step 2: 替换方法体**
  ...
  - `CommandManager.requirePermissionLevel(...)` → `Commands.requirePermissionLevel(...)`
  - `CommandManager.GAMEMASTERS_CHECK` / `MODERATORS_CHECK` / `ADMINS_CHECK` / `OWNERS_CHECK` → `Commands.<同名常量>`（以 Task 0 复核常量名是否变化）
  - `source.hasPermissionLevel(int)` → `source.hasPermission(int)`（Mojang 名，以 Task 0 为准）
...
`//? >=1.21.11` 分支内 `import net.minecraft.world.rule.GameRules;` 与 else 分支 `import net.minecraft.world.GameRules;`，**两者**均改为 Task 0 确认的 Mojang `GameRules` 包路径（预期 `net.minecraft.world.level.GameRules`，以复核为准）。
...
- [ ] **Step 2: 替换方法签名与调用**
  ...
  - `world.getGameRules().getValue(GameRules.PVP)` → `level.getGameRules().getBoolean(GameRules.PVP)`（以 Task 0 复核 getValue/getBoolean 在 Mojang 1.21.11 的形态为准）
...
```java
import net.minecraft.server.MinecraftServer;
import <Task0 确认的 CommandOutput Mojang 全限定名>;   // 预期 net.minecraft.commands.CommandSource
import net.minecraft.commands.CommandSourceStack;
```
...
- [ ] **Step 2: 替换方法体**
  ...
  - `CommandOutput outputCapture` → `<Mojang CommandOutput/CommandSource> outputCapture`（以 Task 0 为准）
  - `server.getCommandManager().parseAndExecute(source, command)` → `server.getCommands()....`（以 Task 0 复核 parseAndExecute 在 Mojang 的方法名/接收者为准）
  - `server.getCommandManager().getDispatcher()` → `server.getCommands().getDispatcher()`
  - `server.getCommandSource()` → `server.createCommandSourceStack()`（以 Task 0 复核为准）
  - `.withOutput(outputCapture)` → 以 Task 0 复核 Mojang 等价 API
...
- [ ] **Step 2: 替换类型与方法调用**
  ...
  - `player.getCommandSource(server.getOverworld())` → `player.createCommandSourceStack(server.overworld())`（1.21.2+ 分支，以 Task 0 为准）；else 分支 `player.getCommandSource()` → `player.createCommandSourceStack()`
  - `.getWorld()` → `.getLevel()`（command source 取 world 的 Mojang 法，以 Task 0 为准）
...
```
→（以 Task 0 复核字段名 `server`/`level` 为准）：
```java
@Mixin(ServerPlayer.class)
public interface ServerPlayerEntityAccessor {
    @Accessor("server")          // 复核后若 Mojang 字段名变化则改
    MinecraftServer getServerInstance();
}

@Mixin(ServerPlayerGameMode.class)
interface ServerPlayerInteractionManagerAccessor {
    @Accessor("level")           // Yarn "world" -> Mojang "level"（复核确认）
...
`@Mixin(MinecraftServer.class)` 目标不变（Mojang 同名）。`method = "loadWorld"` → 以 Task 0 复核 Mojang 方法名（`loadWorld`/`loadLevel`）为准。import `net.minecraft.server.MinecraftServer` 不变。
...
import：`PlayerEntity`→`Player`、`MinecraftServer`不变、`CommandManager`→`Commands`、`ServerCommandSource`→`CommandSourceStack`、`ServerPlayerEntity`→`ServerPlayer`、`ServerWorld`→`ServerLevel`。
  - `player.getCommandSource(world)`（1.21.2+ 分支）→ `player.createCommandSourceStack(level)`（复核）；else `player.getCommandSource()` → `player.createCommandSourceStack()`
  - 保留 `//? >=1.21.2` 块结构
...
import：`PlayerEntity`→`Player`、`ServerPlayerEntity`→`ServerPlayer`、`ServerWorld`→`ServerLevel`、`Text`→`Component`、`Vec3d`→`Vec3`、`World`→`Level`。
  - `targetPlayer.teleport(world,x,y,z,Set.of(),yaw,pitch,false)`（1.21.2+ 分支）→ `targetPlayer.teleport(level,x,y,z,Set.of(),yaw,pitch,false)`（Mojang teleport 签名以 Task 0 复核为准）
  - `new Vec3d(...)` → `new Vec3(...)`
  - 保留 `//? >=1.21.2` 块结构

- [ ] **Step 2: SummonEntityFunction.java**

import：`Entity`→`net.minecraft.world.entity.Entity`、`EntityType`→`net.minecraft.world.entity.EntityType`、`PlayerEntity`→`Player`、`Registries`→`net.minecraft.core.registries.BuiltInRegistries`、`ServerWorld`→`ServerLevel`、`Identifier`→`ResourceLocation`、`BlockPos`→`net.minecraft.core.BlockPos`、`Vec3d`→`Vec3`。
  - **内联全限定名**（行 ~142）：`net.minecraft.entity.SpawnReason.COMMAND` → `net.minecraft.world.entity.MobSpawnType.COMMAND`（复核常量名）
  - `Registries.ENTITY_TYPE.get(id)` → `BuiltInRegistries.ENTITY_TYPE.get(id)`（复核 registry 名）
  - `type.create(world, ...)` → `type.create(level, ...)`
  - 保留 `//? >=1.21.2` 块结构

- [ ] **Step 3: SetBlockFunction.java**

import：`Block`→`net.minecraft.world.level.block.Block`、`BlockState`→`net.minecraft.world.level.block.state.BlockState`、`PlayerEntity`→`Player`、`Registries`→`BuiltInRegistries`、`ServerWorld`→`ServerLevel`、`Identifier`→`ResourceLocation`、`BlockPos`→`net.minecraft.core.BlockPos`。
  - `Registries.BLOCK.get(Identifier)` → `BuiltInRegistries.BLOCK.get(ResourceLocation)`
  - `world.setBlockState(pos, state)` → `level.setBlock(pos, state)`（Mojang 名以 Task 0 复核为准）

- [ ] **Step 4: NearbyEntitiesFunction.java**

import：`Entity`→`net.minecraft.world.entity.Entity`、`LivingEntity`→`net.minecraft.world.entity.LivingEntity`、`HostileEntity`→`net.minecraft.world.entity.monster.Monster`（复核）、`PassiveEntity`→`net.minecraft.world.entity.AgeableMob`（复核）、`PlayerEntity`→`Player`、`Box`→`net.minecraft.world.phys.AABB`、`Vec3d`→`Vec3`。
  - `world.getEntitiesByClass(cls, box, predicate)` → `level.getEntitiesOfClass(cls, aabb, predicate)`（复核签名）
  - `new Box(...)` → `new AABB(...)`
...
import：`PlayerEntity`→`Player`、`MinecraftServer`不变、`ServerCommandSource`→`CommandSourceStack`、`Text`→`Component`。
  - **内联全限定名**（行 ~320 嵌套 `CommandOutputCapture`）：`implements net.minecraft.server.command.CommandOutput` → `implements <Task0 确认的 Mojang CommandOutput 全限定名>`
